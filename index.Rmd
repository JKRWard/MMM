...
Have a look at the [official website](https://libguides.asu.edu/MarchMammalMadness) for all of the latest updates.
...
fils <- list.files (path = "2026", pattern="*.csv") 
...
for (kk in 1:length(fils)){
  entry <- read.csv(here("2026",fils[kk])) %>% 
    mutate(across(1, ~ str_trim(.x, side = "right")))
...
mast <- read_excel(here("2026_results.xlsx"))
...
for (ii in 1: length(players)){
  ...
  mstr <- master
  #mstr <- ans[[1]]
  rnd1 <- unique(playa[1:33]) %in% mstr[1:33]
  rnd2 <- unique(playa[34:49]) %in% mstr[34:49]
  sweet <- unique(playa[50:57]) %in% mstr[50:57]
  elite <- unique(playa[58:61]) %in% mstr[58:61]
  roar <- unique(playa[62:63]) %in% mstr[62:63]
  win <- unique(playa[64]) %in% mstr[64]
...
leaderboard$Player_name <- str_replace(leaderboard$Player_name,"NES2314", "")
...
fils <- list.files (path = "2025", pattern="*.csv") 
...
#using here means that I can easily read files in a subdirectory
for (kk in 1:length(fils)){
  entry <- read.csv(here("2025",fils[kk]))
  # accounting for the ginko/rhizo team
  if(nrow(entry %>% filter(.[,1] == "Ginkgo")) > 0){
  entry <- stringr::str_replace_all(entry[,1], "Ginkgo", "GinkgoRhizo")
  entry <- as.data.frame(entry)
  } 
  
  if(nrow(entry %>% filter(.[,1] == "Mycorrhizal Fungus")) > 0){
  entry <- stringr::str_replace_all(entry[,1], "Mycorrhizal Fungus", "GinkgoRhizo")
  entry <- as.data.frame(entry)
  }
  fl[[kk]] <- as.vector(entry[,1])
}
...
######################## generating the answers 
mast <- read_excel(here("2025_results.xlsx"))
wild <- mast %>%  select("...2") %>%drop_na() %>% rename(Wildcard = "...2")
a <- select(mast, Round_1...5:Round_5...9) %>% rename(Round_1 = Round_1...5,
                                                      Round_2 = Round_2...6,
                                                      Round_3 = Round_3...7,
...
## NES2314 Vertebrate Biology Leaderboard
```{r}
leaderboard2314 <- filter(leaderboardall, group == "NES2314")
Ben <- filter(leaderboardall, Player_name== "Ben Rowland")
Mark <- filter(leaderboardall, Player_name== "Mark Shirley")
#Jess <- filter(leaderboardall, Player_name== "Jess Ward")
#Simon <- filter(leaderboardall, Player_name== "Simon Maddock")
# add relevant staff members 
leaderboard2314 <- bind_rows(leaderboard2314,Ben, Mark ) %>% select(-group)
leaderboard2314$Player_name <- str_replace(leaderboard2314$Player_name,"NES2314", "")
leaderboard2314 %>% arrange(-Total) %>% 
knitr::kable()
```
...
outcomes$winners <- as.character(outcomes$winners)
staff <- outcomes %>% filter(players=="Ben Rowland"|players=="Mark Shirley"| players=="Jess Ward"| players=="Simon Maddock")

predicted_winners <- outcomes %>% filter(group == "NES2314") %>% 
  bind_rows(., staff) %>% 
  select(winners) %>%
  group_by(winners) %>% 
  count() %>% 
...
rankall2314 <- bind_rows(round1, round2, round3,round4,round5,round6)
source(here::here("geom_bump.R"))
library(cowplot)
library(viridis)
library(ggrepel)
ggplot(rankall2314, aes(Round, Ranking, color = Player_name)) +
  geom_point(size = 7) +
  #geom_text(data = rankall %>% filter(Round == min(Round)),
           # aes(x = 0.5, label = Player_name), position=position_jitter(height=01),size = 4) +
  geom_text_repel(data = rankall2314 %>% filter(Round == max(Round)),
            aes(x = Round, label = Player_name),size = 4, hjust = -0.5) +
  geom_bump(aes(smooth = 8), size = 2) +
  scale_x_continuous(limits = c(1, 7),
                     breaks = seq(1, 6, 1)) +
  ...
  labs(y = "RANK",
       x = "Round") +
  scale_y_reverse(breaks = seq(1, nrow(leaderboard2314),1))+
  scale_color_manual(values = viridis(n = nrow(leaderboard2314)))
```

## Ecosystems Leaderboard
```{r}
...
fils <- list.files (path = "2024", pattern="*.csv") 
...
#using here means that I can easily read files in a subdirectory
for (kk in 1:length(fils)){
  entry <- read.csv(here("2024",fils[kk]))
...
######################## generating the answers 
mast <- read_excel(here("2024_results.xlsx"))
wild <- mast %>%  select("...2") %>%drop_na() %>% rename(Wildcard = "...2")
a <- select(mast, Round_1...5:Round_5...9) %>% rename(Round_1 = Round_1...5,
                                                      Round_2 = Round_2...6,
                                                      Round_3 = Round_3...7,
