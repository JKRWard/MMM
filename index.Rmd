...
Check out [#2019MMM](https://twitter.com/search?f=tweets&vertical=default&q=%232019MMM&src=typd)or [@2019MMMletsgo](https://twitter.com/2019MMMletsgo) for all the latest tweets. or have a look at the [official website](https://libguides.asu.edu/MarchMammalMadness) for all of the latest updates.
...
fils <- list.files (path = "2019MMM", pattern="*.csv") 
...
#using here means that I can easily read files in a subdirectory
for (k in 1:length(fils)){
  fl[[k]] <- read_csv(here("2019BIO2029",fils[k]))
}
